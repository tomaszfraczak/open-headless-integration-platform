# OCIP: Operating Model and Team Topologies

## Document Purpose
This document defines the organizational framework and interaction patterns for teams using the Open Composable Integration Platform (OCIP). Inspired by the "Team Topologies" methodology, this model ensures that the platform accelerates delivery rather than becoming a centralized bureaucratic bottleneck.

---

## 1. Team Definitions and Responsibilities

OCIP distinguishes between two primary types of teams to ensure a clear separation of concerns:

### 1.1. The Platform Team (Platform as a Product)
* **Role:** Acts as a product team that builds and maintains the OCIP infrastructure (Tier A and B).
...
* **Role:** Business-focused teams (e.g., Finance, Logistics, HR) that build integrations (Tier C).
...
### Rationale
Traditional "ticket-based" operating models, where a developer must wait days for a new queue or a firewall rule, are the primary cause of integration project delays. OCIP eliminates this via automation.

### Execution
* **No Tickets for Resources:** Domain teams provision their own integration namespaces, Kafka topics, and API routes using the platform's GitOps templates.
...
### Statement
OCIP enforces a "You Build It, You Run It" (YBIYRI) model for Tier C business logic, while the Platform Team guarantees the underlying infrastructure.

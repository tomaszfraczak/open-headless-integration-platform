...
### Statement
The platform prohibits the use of "local" platform accounts for human users. All access must be authenticated via a centralized Identity Provider (IdP) in the **Security & Identity Layer**.

### Rationale
Managing separate credentials for GitHub, ArgoCD, Grafana, and the API Gateway is insecure and unscalable. By federating the platform with the enterprise's existing Single Sign-On (SSO) system (e.g., Microsoft Entra ID, Okta, or Ping Identity) via OIDC or SAML, we ensure that when an employee leaves the company, their access to the entire integration platform is revoked instantly.

### Execution
* **Tier B Integration:** The platform utilizes Keycloak (or a compatible swapped component) as the central Identity Broker.
* **Group Mapping:** Corporate directory groups (e.g., `AD-Group-Integrators-Finance`) are automatically mapped to internal platform roles and Kubernetes namespaces.

---

## 2. The RBAC Persona Model

The platform enforces a strict "Least Privilege" principle. Access is granted based on predefined personas, ensuring that users only have the permissions necessary for their specific job functions.

| Persona | Description | Scope | Key Permissions |
| :--- | :--- | :--- | :--- |
| **Platform Admin** | Manages the global infrastructure (Tier A & B). | Global | Full cluster access, policy definition, infrastructure provisioning. |
...
### Statement
The Kubernetes `Namespace` is the fundamental unit of isolation and security within the platform. 

### Execution
* **Tenant Segregation:** Each business domain (Tenant) is confined to its own namespace, following the `platform-[domain]-[env]` naming convention. 
...
* **Masking Verification:** Security Auditors periodically review the effectiveness of automated log masking to ensure no PII/PCI data is leaking into the observability stack.
